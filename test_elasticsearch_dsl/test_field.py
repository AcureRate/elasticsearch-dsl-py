...
async def test_custom_field_car_wrap_other_field():
...
async def test_field_from_dict():
...
async def test_multi_fields_are_accepted_and_parsed():
...
async def test_modifying_nested():
    f = field.Nested()
...
async def test_nested_provides_direct_access_to_its_fields():
    f = field.Nested()
    f.field('name', 'text', index='not_analyzed')
...
async def test_field_supports_multiple_analyzers():
...
async def test_multifield_supports_multiple_analyzers():
    f = field.Text(fields={
