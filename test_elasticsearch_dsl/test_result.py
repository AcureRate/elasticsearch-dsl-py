...
async def test_agg_response_is_pickleable(agg_response):
    agg_response.hits
...
async def test_response_is_pickleable(dummy_response):
...
async def test_hit_is_pickleable(dummy_response):
    res = response.Response(Search(), dummy_response)
    hits = pickle.loads(pickle.dumps(res.hits))

    assert hits == res.hits

async def test_response_stores_search(dummy_response):
    s = Search()
    r = response.Response(s, dummy_response)

    assert r._search is s

async def test_attribute_error_in_hits_is_not_hidden(dummy_response):
    def f(hit):
...
async def test_interactive_helpers(dummy_response):
    res = response.Response(Search(), dummy_response)
    hits = res.hits
    h = hits[0]
...
async def test_empty_response_is_false(dummy_response):
    dummy_response['hits']['hits'] = []
    res = response.Response(Search(), dummy_response)

    assert not res

async def test_len_response(dummy_response):
    res = response.Response(Search(), dummy_response)
    assert len(res) == 4

async def test_iterating_over_response_gives_you_hits(dummy_response):
    res = response.Response(Search(), dummy_response)
...
async def test_hits_get_wrapped_to_contain_additional_attrs(dummy_response):
    res = response.Response(Search(), dummy_response)
    hits = res.hits
...
async def test_hits_provide_dot_and_bracket_access_to_attrs(dummy_response):
    res = response.Response(Search(), dummy_response)
    h = res.hits[0]
...
async def test_bucket_response_can_be_iterated_over(agg_response):
...
async def test_bucket_keys_get_deserialized(aggs_data, aggs_search):
