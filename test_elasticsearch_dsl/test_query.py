...
async def test_bool_converts_its_init_args_to_queries():
    q = query.Bool(must=[{"match": {"f": "value"}}])

    assert len(q.must) == 1
    assert q.must[0] == query.Match(f='value')

async def test_two_queries_make_a_bool():
...
async def test_other_and_bool_appends_other_to_must():
    q1 = query.Match(f='value1')
    qb = query.Bool()
...
async def test_bool_and_other_appends_other_to_must():
    q1 = query.Match(f='value1')
    qb = query.Bool()
...
async def test_bool_and_other_sets_min_should_match_if_needed():
...
async def test_empty_bool_has_min_should_match_0():
    assert 0 == query.Bool()._min_should_match

async def test_query_and_query_creates_bool():
    q1 = query.Match(f=42)
    q2 = query.Match(g=47)
...
async def test_match_all_and_query_equals_other():
    q1 = query.Match(f=42)
    q2 = query.MatchAll()
...
async def test_inverted_query_becomes_bool_with_must_not():
    q = query.Match(f=42)

    assert ~q == query.Bool(must_not=[query.Match(f=42)])

async def test_inverted_query_with_must_not_become_should():
...
async def test_inverted_query_with_must_and_must_not():
    q = query.Q('bool',
...
async def test_double_invert_returns_original_query():
    q = query.Match(f=42)

    assert q == ~~q

async def test_bool_query_gets_inverted_internally():
...
async def test_match_all_or_something_is_match_all():
    q1 = query.MatchAll()
    q2 = query.Match(f=42)
...
async def test_or_produces_bool_with_should():
    q1 = query.Match(f=42)
    q2 = query.Match(g='v')
...
async def test_two_bool_queries_append_one_to_should_if_possible():
...
async def test_Q_passes_query_through():
    q = query.Match(f='value1')

    assert query.Q(q) is q

async def test_Q_constructs_query_by_name():
    q = query.Q('match', f='value')

    assert isinstance(q, query.Match)
    assert {'f': 'value'} == q._params

async def test_Q_translates_double_underscore_to_dots_in_param_names():
    q = query.Q('match', comment__author='honza')

    assert {'comment.author': 'honza'} == q._params

async def test_Q_doesn_translate_double_underscore_to_dots_in_param_names():
    q = query.Q('match', comment__author='honza', _expand__to_dot=False)

    assert {'comment__author': 'honza'} == q._params

async def test_Q_constructs_simple_query_from_dict():
    q = query.Q({'match': {'f': 'value'}})

    assert isinstance(q, query.Match)
    assert {'f': 'value'} == q._params

async def test_Q_constructs_compound_query_from_dict():
    q = query.Q(
        {
            "bool": {
                "must": [
...
async def test_Q_raises_error_when_passed_in_dict_and_params():
    with raises(Exception):
        query.Q({"match": {'f': 'value'}}, f='value')

async def test_Q_raises_error_when_passed_in_query_and_params():
    q = query.Match(f='value1')

    with raises(Exception):
        query.Q(q, f='value')

async def test_Q_raises_error_on_unknown_query():
    with raises(Exception):
        query.Q('not a query', f='value')

async def test_match_all_and_anything_is_anything():
    q = query.MatchAll()
...
async def test_function_score_with_functions():
...
async def test_function_score_with_no_function_is_boost_factor():
...
async def test_function_score_to_dict():
    q = query.Q(
        'function_score',
...
async def test_function_score_with_single_function():
    d = {
      'function_score': {
        'filter': {"term": {"tags": "python"}},
        'script_score': {
...
async def test_function_score_from_dict():
    d = {
      'function_score': {
        'filter': {"term": {"tags": "python"}},
        'functions': [
