...
async def test_meta_from_dict():
    max_score = aggs.Max(field='score')
    a = aggs.A('terms', field='tags', aggs={'max_score': max_score}, meta={'some': 'metadata'})

    assert aggs.A(a.to_dict()) == a

async def test_A_creates_proper_agg():
    a = aggs.A('terms', field='tags')

    assert isinstance(a, aggs.Terms)
    assert a._params == {'field': 'tags'}

async def test_A_handles_nested_aggs_properly():
    max_score = aggs.Max(field='score')
    a = aggs.A('terms', field='tags', aggs={'max_score': max_score})
...
async def test_A_passes_aggs_through():
    a = aggs.A('terms', field='tags')
    assert aggs.A(a) is a

async def test_A_from_dict():
    d = {
        'terms': {'field': 'tags'},
...
async def test_A_fails_with_incorrect_dict():
    correct_d = {
        'terms': {'field': 'tags'},
        'aggs': {'per_author': {'terms': {'field': 'author.raw'}}},
    }
...
async def test_A_fails_with_agg_and_params():
    a = aggs.A('terms', field='tags')

    with raises(Exception):
        aggs.A(a, field='score')

async def test_buckets_are_nestable():
...
async def test_metric_inside_buckets():
    a = aggs.Terms(field='tags')
...
async def test_buckets_equals_counts_subaggs():
    a = aggs.Terms(field='tags')
...
async def test_buckets_to_dict():
    a = aggs.Terms(field='tags')
    a.bucket('per_author', 'terms', field='author.raw')
...
async def test_nested_buckets_are_reachable_as_getitem():
    a = aggs.Terms(field='tags')
    b = a.bucket('per_author', 'terms', field='author.raw')
...
async def test_nested_buckets_are_settable_as_getitem():
    a = aggs.Terms(field='tags')
    b = a['per_author'] = aggs.A('terms', field='author.raw')

    assert a.aggs['per_author'] is b

async def test_filter_can_be_instantiated_using_positional_args():
...
async def test_filter_aggregation_as_nested_agg():
    a = aggs.Terms(field='tags')
...
async def test_filter_aggregation_with_nested_aggs():
    a = aggs.Filter(query.Q('term', f=42))
...
async def test_filters_correctly_identifies_the_hash():
