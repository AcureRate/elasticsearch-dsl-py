...
async def bulk(client, actions, stats_only=False, **kwargs):
    """
    ...
    async for ok, item in streaming_bulk(client, actions, **kwargs):
...
async def _process_bulk_chunk(client, bulk_actions, raise_on_exception=True, raise_on_error=True, **kwargs):
    """
...
async def streaming_bulk(client, actions, chunk_size=500, max_chunk_bytes=100 * 1024 * 1024,
    ...
    for bulk_actions in _chunk_actions(actions, chunk_size, max_chunk_bytes, client.transport.serializer):
        async for result in _process_bulk_chunk(client, bulk_actions, raise_on_exception, raise_on_error, **kwargs):
            yield result
...
async def parallel_bulk(client, actions, thread_count=4, chunk_size=500,
